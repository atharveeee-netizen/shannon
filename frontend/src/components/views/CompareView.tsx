          ...
          <table className="w-full text-left text-xs font-mono">
            ...
            <tbody className="divide-y divide-border">
              {benchmarkMatrix.map((item) => {
                ...
                return (
                  <tr key={item.hardware_id} className={`hover:bg-surface-raised/40 transition-colors ${isCurrent ? 'bg-surface-raised' : ''}`}>
                    ...
                    <td className="py-3 px-4">
                      {item.fits ? (
                        ...
                        <span className="px-2 py-0.5 rounded bg-rose-500/10 border border-rose-500/20 text-rose-400 text-[11px] font-medium inline-flex items-center gap-1">
                          <AlertTriangle className="w-3 h-3" /> EXCEEDS MCU
                        </span>
                      )}
                    </td>
                    <td className="py-3 px-4">
                      {!isCurrent && (
                        <button
                          ...
                          className="px-2.5 py-1 rounded-md bg-surface border border-border hover:border-primary text-text-primary text-xs cursor-pointer transition-colors"
                        >
                          Select
                        </button>
                      )}
